...
class COCOAnnotationTool:
    ...
    def setup_ui(self):
        ...
        # Damage Severity
        ttk.Label(form_frame, text="Damage Severity (1-4):").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.severity_var = tk.IntVar(value=1)
        severity_frame = ttk.Frame(form_frame)
        severity_frame.grid(row=1, column=1, sticky=tk.W, pady=1)
        self.severity_scale = ttk.Scale(severity_frame, from_=1, to=4, variable=self.severity_var,
                                        orient=tk.HORIZONTAL, length=150)
        self.severity_scale.pack(side=tk.LEFT)
        self.severity_label = ttk.Label(severity_frame, text="1", width=3)
